    ...
    def setForeground(self, color):
        ...
        try:
            if color == 'red':
                ...
                self.setBackground(COLOR_RED)
            elif color == 'green':
                self.setForeground(QtGui.QColor('black'))
                self.setBackground(COLOR_GREEN)
        except ValueError:
            pass
...
########################################################################
class StatusCell(QtWidgets.QTableWidgetItem):
    """状态单元格"""
...
class OrderMonitor(BasicMonitor):
    ...
    def __init__(self, mainEngine, eventEngine, parent=None):
        ...
        d['vtOrderID'] = {'chinese':vtText.ORDER_ID, 'cellType':NumCell}
        d['symbol'] = {'chinese':vtText.CONTRACT_SYMBOL, 'cellType':BasicCell}
        d['name'] = {'chinese':'名称', 'cellType':BasicCell}
        d['strategy'] = {'chinese':'策略', 'cellType':BasicCell}
        d['riskCtrl'] = {'chinese':'风控', 'cellType':BasicCell}
        d['gatewayName'] = {'chinese':vtText.GATEWAY, 'cellType':BasicCell}
        d['status'] = {'chinese':'运行状态', 'cellType':BasicCell}
        self.setHeaderDict(d)
...
class TradingWidget(QtWidgets.QFrame):
    ...
    productClassList = [PRODUCT_EQUITY,
                        PRODUCT_EQUITY,
                        PRODUCT_FUTURES,
                        PRODUCT_OPTION,
                        PRODUCT_FOREX]
...
# chenzejun modify
########################################################################
class TaskMonitor(BasicMonitor):
    """交易任务监控"""

    #----------------------------------------------------------------------
    def __init__(self, mainEngine, eventEngine, parent=None):
        """Constructor"""
        super(TaskMonitor, self).__init__(mainEngine, eventEngine, parent)

        self.mainEngine = mainEngine

        d = OrderedDict()
        d['taskID'] = {'chinese':'任务ID', 'cellType':NumCell}
        d['symbol'] = {'chinese':'股票代码', 'cellType':BasicCell}
        d['name'] = {'chinese':'股票名称', 'cellType':BasicCell}
        #d['vtSymbol'] = {'chinese':'系统代码', 'cellType':BasicCell}
        d['strategyName'] = {'chinese':'策略', 'cellType':BasicCell}
        d['riskCtrlName'] = {'chinese':'风控', 'cellType':BasicCell}
        d['gatewayName'] = {'chinese':vtText.GATEWAY, 'cellType':BasicCell}
        d['status'] = {'chinese':'运行状态', 'cellType':BasicCell}
        self.setHeaderDict(d)

        self.setDataKey('taskID')
        self.setEventType(EVENT_TASK)
        self.setFont(BASIC_FONT)
        self.setSaveData(True)
        self.setSorting(True)
    ...
    def initMenu(self):
        ...
        delAction = QtWidgets.QAction('删除任务', self)
        delAction.triggered.connect(self.deleteSelectTask)
    ...
    def deleteSelectTask(self):
        ...
        for index in select_rows:
            pos = self.item(index, 0).data
            key = pos.strategyName + pos.riskCtrlName + pos.gatewayName + pos.symbol
            task = self.mainEngine.delTask(key)


            self.removeRow(index)
            print('deleteSelectTask, key:', key)
    ...
    def connectSignal(self):
        ...
        self.itemDoubleClicked.connect(self.stopSelectTask)

    #----------------------------------------------------------------------
    def stopSelectTask(self, cell):
        """根据单元格的数据撤单"""
        ...
        if cell.text() == 'running':
            cell.setForeground('red')
            cell.setText('stop')

            task = self.mainEngine.stopTask(key)

        elif cell.text() == 'stop':
            cell.setForeground('green')
            cell.setText('running')

            task = self.mainEngine.startTask(key)

        else:
            return

        if task is None:
            print(cell.text(), ',task is none', key)
            return
        task.status = cell.text()
...
class TaskWidget(QtWidgets.QFrame):
    ...
    def __init__(self, mainEngine, eventEngine, parent=None):
        ...
        # 添加交易接口
        l = mainEngine.getAllGatewayDetails()
        gatewayNameList = [d['gatewayDisplayName'] for d in l]
        self.gatewayList.extend(gatewayNameList)
    ...
    def initUi(self):
        ...
        labelStartegy = QtWidgets.QLabel('策略')   # 策略
        labelRiskCtrl = QtWidgets.QLabel('风控')
        labelGateway = QtWidgets.QLabel(vtText.GATEWAY)

        self.lineSymbol = QtWidgets.QLineEdit()
        self.lineName = QtWidgets.QLineEdit()

        self.comboStrategy = QtWidgets.QComboBox()
        self.comboStrategy.addItems(self.stragegyList)

        self.comboRiskCtrl = QtWidgets.QComboBox()
        self.comboRiskCtrl.addItems(self.riskCtrlList)
        ...
        gridleft.addWidget(labelSymbol, 0, 0)
        gridleft.addWidget(labelName, 1, 0)
        gridleft.addWidget(labelStartegy, 2, 0)
        gridleft.addWidget(labelRiskCtrl, 3, 0)
        gridleft.addWidget(labelGateway, 4, 0)

        gridleft.addWidget(self.lineSymbol, 0, 1, 1, -1)
        gridleft.addWidget(self.lineName, 1, 1, 1, -1)
        gridleft.addWidget(self.comboStrategy, 2, 1, 1, -1)
        gridleft.addWidget(self.comboRiskCtrl, 3, 1, 1, -1)
        gridleft.addWidget(self.comboGateway, 4, 1, 1, -1)
        ...
        # 关联更新, button
        buttonSendOrder.clicked.connect(self.sendTask)
        buttonCancelAll.clicked.connect(self.cancelAll)
        #self.lineSymbol.returnPressed.connect(self.updateSymbol)
    ...
    def sendTask(self):
        ...
        print(__name__, self.__doc__)
        symbol = str(self.lineSymbol.text())
        #name = str(self.lineSymbol.text())
        strategyDisplayName = str(self.comboStrategy.currentText())
        riskCtrlDisplayName = str(self.comboRiskCtrl.currentText())
        gatewayDisplayName = str(self.comboGateway.currentText())


        if self.mainEngine.objectIsValid(symbol) == False:
            print('symbol is invalid, symbol:', symbol)
            self.mainEngine.writeLog('symbol is invalid, symbol:' + symbol)
            return

        #name = self.mainEngine.objectGetValue(symbol)
        ...
        task.symbol = symbol
        task.name = name
        task.strategyName = self.mainEngine.getStrategyName(strategyDisplayName)
        task.riskCtrlName = self.mainEngine.getRiskCtrlName(riskCtrlDisplayName)
        task.gatewayName = self.mainEngine.getGatewayName(gatewayDisplayName)
        task.status = 'running'


        #启动交易任务
        #获取app engine, 多实例运行
        print('button, sendTask,', strategyDisplayName, riskCtrlDisplayName, gatewayDisplayName)

        # 创建应用实例
        exist = self.mainEngine.newTask(task)
        if exist != 0:   # 已经有任务实例
            return

        #发送 交易任务 到监控单元进行显示
        # 通用事件
        event1 = Event(type_=EVENT_TASK)
        event1.dict_['data'] = task
        self.mainEngine.eventEngine.put(event1)


        print('start task,', strategyDisplayName, riskCtrlDisplayName, gatewayDisplayName)
        key = task.strategyName + task.riskCtrlName + task.gatewayName + task.symbol
        self.mainEngine.startTask(key)
    ...
    def keyReleaseEvent(self, event):
        """重载键盘事件"""
        symbol = str(self.lineSymbol.text())
        #print('key release', symbol, len(symbol))
        if len(symbol) == 6:
            if self.mainEngine.objectIsValid(symbol) == False:
                print('symbol is invalid, symbol:', symbol)
