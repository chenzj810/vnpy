...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def initCentral(self):
        ...
        widgetTaskM, dockTaskM = self.createDock(TaskMonitor, vtText.TRADE_TASK, QtCore.Qt.RightDockWidgetArea)
        ...
        widgetTaskW, dockTaskW = self.createDock(TaskWidget, vtText.TRADING, QtCore.Qt.LeftDockWidgetArea)
        ...
        # 连接组件之间的信号
        widgetPositionM.itemDoubleClicked.connect(widgetTaskW.closePosition)
    ...
    def initMenu(self):
        ...
        sysMenu.addSeparator()
        sysMenu.addAction(self.createAction(vtText.CONNECT_DATABASE, self.mainEngine.historyData.historyDataConnect, loadIconPath('database.ico')))
        sysMenu.addSeparator()
        ...
        for appDetail in self.appDetailList:
            print(appDetail['appDisplayName'], appDetail['appIco'])
        ...
        '''
        # strategy
        for appDetail in self.strategyDetailList:
            #function = self.createOpenAppFunction(appDetail)
            action = self.createAction(appDetail['appDisplayName'], self.openStrategyFunction, loadIconPath(appDetail['appIco']))
            appMenu.addAction(action)


        # risk
        for appDetail in self.riskCtrlDetailList:
            #function = self.createOpenAppFunction(appDetail)
            action = self.createAction(appDetail['appDisplayName'], self.openRiskFunction, loadIconPath(appDetail['appIco']))
            appMenu.addAction(action)
        '''
        # end chenzejun
    ...
    def addConnectAction(self, menu, gatewayName, displayName=''):
        ...
        print('addConnectAction', actionName)
    ...
    def createOpenAppFunction(self, appDetail):
        ...
        def openAppFunction():
            ...
            try:
                ...
                #app = self.mainEngine.appDict[appName]
                print('open app, KeyError', KeyError)
                self.widgetDict[appName] = appDetail['appWidget']('', self.eventEngine)
                self.widgetDict[appName].show()
    ...
    #----------------------------------------------------------------------
    """创建打开应用UI的函数"""
    def openStrategyFunction(self, appDetail):
        appName = appDetail['appName']
        print('open app', appName)
        try:
            self.widgetDict[appName].show()
    ...
    #----------------------------------------------------------------------
    """创建打开应用UI的函数"""
    def openRiskFunction(self, appDetail):
        appName = appDetail['appName']
        print('open app', appName)
        try:
            self.widgetDict[appName].show()
        except KeyError:
            appEngine = self.mainEngine.riskCtrlDict[appName]
            self.widgetDict[appName] = appDetail['appWidget'](appEngine, self.eventEngine)
            self.widgetDict[appName].show()
