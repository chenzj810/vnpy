...
from vnpy.api.stub import vnstub
...
SYMBOL_MAP = {}
SYMBOL_MAP[(vnstub.COINTYPE_BTC, 'cny')] = SYMBOL_BTCCNY
SYMBOL_MAP[(vnstub.COINTYPE_LTC, 'cny')] = SYMBOL_LTCCNY
SYMBOL_MAP[(vnstub.COINTYPE_BTC, 'usd')] = SYMBOL_BTCUSD
...
########################################################################
class StubGateway(VtGateway):
    """火币接口"""

    #----------------------------------------------------------------------
    def __init__(self, eventEngine, gatewayName='STUB'):
        """Constructor"""
        super(StubGateway, self).__init__(eventEngine, gatewayName)
        
        self.market = 'cny'
        
        self.tradeApi = StubTradeApi(self)
        self.dataApi = StubDataApi(self)
...
class StubDataApi(vnstub.DataApi):
    ...
    def connect(self, interval, market, debug=False):
        ...
        # 订阅行情并推送合约信息
        if self.market == vnstub.MARKETTYPE_CNY:
            self.subscribeQuote(vnstub.SYMBOL_BTCCNY)
            self.subscribeQuote(vnstub.SYMBOL_LTCCNY)

            self.subscribeDepth(vnstub.SYMBOL_BTCCNY, 5)
            self.subscribeDepth(vnstub.SYMBOL_LTCCNY, 5)

            contract = VtContractData()
            contract.gatewayName = self.gatewayName
            contract.symbol = SYMBOL_BTCCNY
            contract.exchange = EXCHANGE_STUB
            ...
            self.subscribeQuote(vnstub.SYMBOL_BTCUSD)

            self.subscribeDepth(vnstub.SYMBOL_BTCUSD)
