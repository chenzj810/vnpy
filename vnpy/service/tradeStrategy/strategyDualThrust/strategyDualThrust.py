...
from vnpy.service.tradeStrategy.vtStrategyManager import StrategyTemplate
...
class DualThrustStrategy(StrategyTemplate):
    ...
    #----------------------------------------------------------------------
    def __init__(self, mainEngine, setting=None):
        """Constructor"""
        super(DualThrustStrategy, self).__init__(mainEngine, setting)
    ...
    def onBar(self, bar):
        ...
        if bar.datetime.time() < self.exitTime:
            if self.pos == 0:
                if bar.close > self.dayOpen:
                    if not self.longEntered:
                        self.onBuy(self.longEntry, self.fixedSize, stop=True)
                else:
            ...
            # 持有多头仓位
                ...
                # 多头止损单
                self.onSell(self.shortEntry, self.fixedSize, stop=True)
