...
3. 将IF0000_1min.csv用stubHistoryData.py导入MongoDB后，直接运行本文件即可回测策略
...
from vnpy.service.app.stubStrategy.stubTemplate import StubTemplate


########################################################################
class AtrRsiStrategy(StubTemplate):
    ...
    def __init__(self, stubEngine, setting):
        """Constructor"""
        super(AtrRsiStrategy, self).__init__(stubEngine, setting)
    ...
    def onInit(self):
        """初始化策略（必须由用户继承实现）"""
        self.writeStubLog('%s策略初始化' %self.name)
    ...
    def onStart(self):
        """启动策略（必须由用户继承实现）"""
        self.writeStubLog('%s策略启动' %self.name)
        self.putEvent()

    #----------------------------------------------------------------------
    def onStop(self):
        """停止策略（必须由用户继承实现）"""
        self.writeStubLog('%s策略停止' %self.name)
        self.putEvent()
