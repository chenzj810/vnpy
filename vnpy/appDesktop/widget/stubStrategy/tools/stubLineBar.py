...
class StubLineBar(object):
    ...
    self.lineM = StubLineBar(self, self.onBar, lineMSetting)
    ...
    def onTick(self, tick):
        ...
        #    self.writeStubLog(u'无效的tick时间:{0}'.format(tick.datetime))
        #    return

        if tick.datetime.hour == 8 or tick.datetime.hour == 20:
            self.writeStubLog('竞价排名tick时间:{0}'.format(tick.datetime))
            return
    ...
    def __drawLineBar(self, tick):
        ...
        if (tick.datetime.hour == 9 or tick.datetime.hour == 21) \
            ...
            self.writeStubLog('line Bar jumpbars:{0}'.format(jumpBars))
    ...
    def __recountPreHighLow(self):
        ...
        if len(self.lineBar) < self.inputPreLen:
            self.writeStubLog('数据未充分,当前Bar数据数量：{0}，计算High、Low需要：{1}'.
    ...
    def __recountEma(self):
        ...
        if len(self.lineBar) < max(7, self.inputEma1Len, self.inputEma2Len)+2:
            self.debugStubLog('数据未充分,当前Bar数据数量：{0}，计算EMA需要：{1}'.
    ...
    def __recountDmi(self):
        ...
        if len(self.lineBar) < self.inputDmiLen+1:
            self.debugStubLog('数据未充分,当前Bar数据数量：{0}，计算DMI需要：{1}'.format(len(self.lineBar), self.inputDmiLen+1))
            return
        ...
        if self.barPdi > self.barMdi and self.barAdxTrend and self.barAdxrTrend and self.barPdi >= self.inputDmiMax:
            ...
            self.writeStubLog('{0}[DEBUG]Buy Signal On Bar,Pdi:{1}>Mdi:{2},adx[-1]:{3}>Adx[-2]:{4}'
        ...
        if self.barPdi < self.barMdi and self.barAdxTrend and self.barAdxrTrend and self.barMdi >= self.inputDmiMax:
            ...
            self.writeStubLog('{0}[DEBUG]Short Signal On Bar,Pdi:{1}<Mdi:{2},adx[-1]:{3}>Adx[-2]:{4}'
                                 .format(self.curTick.datetime, self.barPdi, self.barMdi, self.lineAdx[-1], self.lineAdx[-2]))
        else:
    ...
    def __recountAtr(self):
        ...
        if len(self.lineBar) < maxAtrLen+1:
            self.debugStubLog('数据未充分,当前Bar数据数量：{0}，计算ATR需要：{1}'.
    ...
    def __recoundAvgVol(self):
        ...
        if len(self.lineBar) < self.inputVolLen+1:
            self.debugStubLog('数据未充分,当前Bar数据数量：{0}，计算Avg Vol需要：{1}'.
    ...
    def __recountRsi(self):
        ...
        if len(self.lineBar) < self.inputRsiLen+2:
            self.debugStubLog('数据未充分,当前Bar数据数量：{0}，计算RSI需要：{1}'.
    ...
    def __recountCmi(self):
        ...
        if len(self.lineBar) < self.inputCmiLen:
            self.debugStubLog('数据未充分,当前Bar数据数量：{0}，计算CMI需要：{1}'.
    ...
    def __recountBoll(self):
        ...
        if l < min(7, self.inputBollLen)+1:
            self.debugStubLog('数据未充分,当前Bar数据数量：{0}，计算Boll需要：{1}'.
    ...
    def writeStubLog(self, content):
        """记录STUB日志"""
        self.strategy.writeStubLog('['+self.name+']'+content)

    def debugStubLog(self,content):
        """记录STUB日志"""
        if DEBUGSTUBLOG:
            self.strategy.writeStubLog('['+self.name+'-DEBUG]'+content)